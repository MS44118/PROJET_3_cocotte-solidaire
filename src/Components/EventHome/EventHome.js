...
function EventHome() {
  ...
  const [filterManger, setFilterManger] = useState([]);
  ...
  useEffect(() => {
    let array = [];
    array = events.filter(event => event.name_event === 'manger');
    setFilterManger({ visible: true, arrayManger: array });
  }, [events]);

  //
  // useEffect(() => {
  //   let array = [];
  //   array = events.map((event) => { return (event.name_event === 'manger' ? true : null); });
  //   setFilterManger(array);
  // }, [events]);
    ...
    <div>
      ...
      <form action="#">
        ...
        <p>
          <label htmlFor="checkManger">
            <input type="checkbox" className="filled-in" checked={setFilterManger({ visible: false, arrayManger: filterManger.arrayManger })} />
            <span>
              {'Manger: '}
            </span>
          </label>
      ...
      <div className="events-registrations-list container">
        ...
        {events
          ...
          // .filter(event => event.name_event === 'manger' || 'cuisiner & manger' || 'others')
          // 0 = all
          // 1 = 'manger'
          // 2 = 'cuisiner & manger'
          // 3 = others
